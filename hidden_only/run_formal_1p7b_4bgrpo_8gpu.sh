#!/usr/bin/env bash
# Direct formal run for OPRD-Bridge hidden-only on ALFWorld.
...
export TMPDIR="${TMPDIR:-$run_root/tmp_hid_1p7b_4bgrpo_$run_id}"
export TEMP="$TMPDIR"
export TMP="$TMPDIR"
export RAY_TMPDIR="${RAY_TMPDIR:-/tmp/rayhid4b_${run_id}_$$}"
