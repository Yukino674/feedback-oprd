...
class ATODRayTrainer(SODRayTrainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # OPRD-Bridge hidden supervision. Default OFF: original ATOD/OPD behavior
        # is unchanged unless algorithm.atod.hidden_signal.enabled=true.
        hidden_cfg = atod_cfg.get("hidden_signal", {})
        ...
        self.oprd_bridge_hidden_micro_batch_size = hidden_cfg.get("micro_batch_size", 1)
        ...
        if self.use_oprd_bridge_hidden_signal and not self.oprd_bridge_checkpoint_path:
            raise ValueError(
                "algorithm.atod.hidden_signal.enabled=true requires "
                "algorithm.atod.hidden_signal.bridge_checkpoint_path"
            )
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("teacher_forward", timing_raw):
                        if self.use_oprd_bridge_hidden_signal:
                            teacher_hidden = self._compute_teacher_oprd_bridge_hidden(batch)
                            batch = batch.union(teacher_hidden)
                            metrics["oprd_bridge/enabled"] = 1.0
                            metrics["oprd_bridge/teacher_target_shape_layers"] = float(
                                batch.batch["teacher_oprd_bridge_repr"].shape[1]
                            )
                            metrics["oprd_bridge/teacher_target_shape_rank"] = float(
                                batch.batch["teacher_oprd_bridge_repr"].shape[-1]
                            )
                            if self.oprd_bridge_disable_logprob_opd:
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        if self.use_oprd_bridge_hidden_signal and self.oprd_bridge_disable_logprob_opd:
                            teacher_lp = student_log_probs.detach()
                            kl_coef_for_adv = 0.0
                        else:
                            teacher_lp = batch.batch["teacher_log_probs"]
                            kl_coef_for_adv = self.atod_kl_coef
                        ...
                        if self.use_oprd_bridge_hidden_signal:
                            ...
                            metrics["oprd_bridge/logprob_opd_disabled"] = (
                                1.0 if self.oprd_bridge_disable_logprob_opd else 0.0
                            )
