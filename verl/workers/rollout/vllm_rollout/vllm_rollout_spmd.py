...
class vLLMRollout(BaseRollout):
    def __init__(self, model_path: str, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        enable_sleep_mode = bool(config.get("enable_sleep_mode", True))
        ...
        # Offload vllm model to reduce peak memory usage when this engine is
        # allowed to participate in vLLM sleep/wake memory-pool management.
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if response_length <= 0 or response_length > self.config.response_length:
            raise ValueError(
                f"response_length must be in [1, {self.config.response_length}], "
