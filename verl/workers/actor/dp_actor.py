...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _oprd_bridge_project_micro_batch(
        ...
        *,
        ...
        Returns [batch, paired_layers, response_len, rank].
        """
        ...
        response_len = int(response_mask.size(-1))
        ...
        projected = []
        for pair_idx, (student_layer_idx, teacher_layer_idx) in enumerate(bank["layer_pairs"]):
            if mode == "student":
                ...
                h = hidden_states[layer_idx][:, -response_len:, :]
    ...
    def compute_oprd_bridge_hidden(self, data: DataProto, *, checkpoint_path: str, mode: str) -> torch.Tensor:
        ...
        for micro_batch in micro_batches:
            with torch.no_grad():
                hidden = self._oprd_bridge_project_micro_batch(
                    micro_batch,
    ...
    def _oprd_bridge_hidden_loss(
        ...
        *,
        ...
        student_repr = self._oprd_bridge_project_micro_batch(
            micro_batch,
            checkpoint_path=checkpoint_path,
            mode="student",
        )
        ...
        mask = micro_batch["response_mask"].to(per_token_loss.device, dtype=per_token_loss.dtype)
        loss = (per_token_loss * mask).sum() / mask.sum().clamp_min(eps)
        scaled = loss * float(coef)
        with torch.no_grad():
            loss_value = float(loss.detach().item())
            scaled_value = float(scaled.detach().item())
            valid = mask.bool()
            metrics = {
                "actor/oprd_bridge_hidden_loss": loss_value,
                "actor/oprd_bridge_hidden_loss_scaled": scaled_value,
                "actor/oprd_bridge_hidden_loss_x1e6": loss_value * 1e6,
                "actor/oprd_bridge_hidden_loss_scaled_x1e6": scaled_value * 1e6,
