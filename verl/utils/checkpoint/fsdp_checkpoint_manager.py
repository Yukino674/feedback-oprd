...
# ── Host-RAM reclaim helper ───────────────────────────────────────────────────
# On glibc, freeing large Python objects (e.g. the CPU-gathered fp32 state_dicts
# during checkpoint save) does NOT return memory to the OS: glibc's per-arena
# free lists hoard it (worsened on high-core boxes -> up to 8*ncores arenas).
# gc.collect() drops the Python refs but RSS stays high until glibc trims. We
# force the trim so the freed pages are returned immediately, BEFORE the next
# step's vLLM wake-up (rollout_mode_enter) spikes host RAM again. Resolved once;
# degrades to a no-op if libc/malloc_trim is unavailable.
try:
...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert "model" in checkpoint_contents and "extra" in checkpoint_contents, f"FSDPCheckpointManager must include ['model', 'extra'], got {checkpoint_contents}"
    ...
    def load_checkpoint(self, local_path: str, hdfs_path: str = None, del_local_after_load=False):
        ...
        # optimizer shard may be absent if the checkpoint was saved without it
        ...
        if os.path.exists(remote_optim_path):
            ...
            print(f"[rank-{self.rank}]: optim shard not found at {remote_optim_path}, skip loading optimizer state")
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        with warnings.catch_warnings():
            ...
            with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
                save_optimizer = "optimizer" in self.checkpoint_contents
                model_state_dict = self.model.state_dict()
                optimizer_state_dict = self.optimizer.state_dict() if (self.optimizer is not None and save_optimizer) else None
                ...
                if optimizer_state_dict is not None:
                    ...
                    print(f"[rank-{self.rank}]: Skip saving optim (optimizer not in checkpoint_contents)")
                torch.save(extra_state_dict, extra_path)

                # Release the CPU-gathered shards IMMEDIATELY after writing them, while
                # still inside this rank's save scope. Deferring to the end of the function
                # keeps ~114GB (8 ranks x ~14GB) of fp32 buffers resident across rank0's HF
                # dump + the dist.barrier, and that residency is exactly what collides with
                # the next step's vLLM wake-up to breach the OOM threshold. Drop refs, GC,
                # then hand the pages back to the OS via malloc_trim.
        ...
        # Explicitly release the CPU-gathered state dicts and reclaim host memory.
        # Without this, the large model/optimizer state_dicts aggregated to CPU during
        # save linger until the next GC, permanently inflating host RAM (observed ~+100GB
        # that never falls back), which eventually drives the colocated rollout into CPU OOM.
        try:
            del model_state_dict
        except NameError:
            pass
