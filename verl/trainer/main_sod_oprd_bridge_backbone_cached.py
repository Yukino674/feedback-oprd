"""
Main entry point for SOD-style OPRD-Bridge hidden supervision.
...
class SODOPRDBridgeTaskRunner:
    def run(self, config):
        ...
        # This variant always uses the external teacher worker, but uses it for
        # OPRD-Bridge hidden targets rather than log-prob OPD supervision.
        ...
        if config.actor_rollout_ref.actor.strategy in ["fsdp", "fsdp2"]:
            ...
            from verl.workers.fsdp_workers_oprd_bridge_backbone_only import ActorRolloutRefWorker, AsyncActorRolloutRefWorker, CriticWorker
        ...
        if config.reward_model.enable:
            if config.reward_model.strategy in ["fsdp", "fsdp2"]:
                from verl.workers.fsdp_workers_oprd_bridge_hidden_only import RewardModelWorker
        ...
        from verl.trainer.ppo.sod_oprd_bridge_cached_ray_trainer import SODOPRDBridgeCachedRayTrainer

        trainer = SODOPRDBridgeCachedRayTrainer(
            config=config,
            tokenizer=tokenizer,
            processor=processor,
